...
internal static class PostgreSqlProviderCases
{
    ...
    public static DatabaseProviderCase RedshiftProtocol { get; } = CreatePostgreSqlCompatible(
        ...
        "This exercises GhostShell's Redshift pgwire path only; native Redshift needs a managed AWS endpoint.");
    ...
    private static DatabaseProviderCase CreatePostgreSqlCompatible(
        ...
        new ContainerDatabaseProviderCase(
            ...
            [],
            static (host, port) =>
                $"Host={host};Port={port};Database=ghostshell;Username=ghostshell;Password={Password};SSL Mode=Disable;Timeout=15;Command Timeout=30",
            "SELECT 1",
...
, StringComparison.Ordinal) ? null
    ...
    private static DatabaseSeed CreatePostgreSqlSeed() => new(
        [
            "CREATE SCHEMA ghostshell_extra",
            """
            CREATE FUNCTION ghostshell_extra.viewer_identity(value BIGINT)
            RETURNS BIGINT
            LANGUAGE SQL
            IMMUTABLE
            RETURN value
