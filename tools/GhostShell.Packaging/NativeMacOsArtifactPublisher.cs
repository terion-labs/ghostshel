...
internal sealed record NativeMacOsArtifactPublishCommand(
    ...
    string DestinationDirectory)
{
    ...
    public static NativeMacOsArtifactPublishCommand Parse(
        IReadOnlyList<string> arguments)
    {
        var values = NativeMacOsEvidenceCommandParser.Parse(arguments, Options);
        return new NativeMacOsArtifactPublishCommand(
            NativeMacOsEvidenceCommandParser.Required(
                values,
                "--staged-directory"),
            NativeMacOsEvidenceCommandParser.Required(
                values,
...
internal sealed record NativeMacOsArtifactPublishResult(
    string DestinationDirectory,
...
/// </summary>
internal static class NativeMacOsArtifactPublisher
{
    private const string ArtifactDirectoryName = "osx-arm64";
    ...
    public static NativeMacOsArtifactPublishResult Publish(
        string stagedDirectory,
        string destinationDirectory)
    {
        ...
        return new NativeMacOsArtifactPublishResult(
            destination.Path,
            destination.Exists);
    }
    ...
    private static DestinationState ParseDestination(string path)
    {
                ...
                StringComparison.Ordinal))
        {
            throw new ArgumentException(
                $"The native artifact destination must be named "
                + $"{ArtifactDirectoryName}.",
                nameof(path));
        }
        ...
        string destination)
    {
        ...
        {
            throw new ArgumentException(
                $"The staged native artifact directory must be named "
                + $"{ArtifactDirectoryName}.",
                nameof(staged));
        }
    ...
    private static void ValidateTree(string rootPath, string label)
    {
        ...
        NativeMacOsPath.ValidatePortableUniqueness(paths);
        RequireSafeDirectory(root, label);
    }
