...
public sealed partial class FilePanelClient : IFilePanelClient, IFileTransferQueueClient, IDisposable
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var (kind, mediaType) = ClassifyPreview(source, content);
    ...
    private static (FilePanelPreviewKind Kind, string MediaType) ClassifyPreview(
        FilePanelLocation location,
        byte[] content)
    {
        if (IsPng(content))
    ...
    private static bool TryDecodeText(byte[] content, out string text)
    {
        try
        {
    ...
    {
        var extensionSuggestsJson = location.Address is FilePanelAddress.Hierarchical hierarchical
            && hierarchical.Path.Name?.Value.EndsWith(".json", StringComparison.OrdinalIgnoreCase) == true;
