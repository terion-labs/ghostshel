...
///
/// A page at a time, by path: a document is opened once and only the page being
/// looked at is rasterized, so a thousand-page file costs the same as a
/// one-page file until the user pages into it.
/// </summary>
public interface IPdfPreviewRenderer
{
    ...
    ValueTask<int> CountPagesAsync(string path, CancellationToken cancellationToken);
