...
- Every MCP tool is the trusted generic `mcp.call` mutation under `McpTools`.
  `Off` opens no MCP transport. `Ask` and `Auto` both require an
...
- `YOLO` is explicit, confirmed, target-scoped, run-bounded, immediately
  revocable, and audited. It is never inferred from a missing UI. The runtime
  permits terminal YOLO for the confirmed run scope when that scope contains a
  terminal. Workspace, internal `OpenTab`, selected-panel, and exact-panel
  targets remain bound by the broker to that immutable run target.
...
14. An MCP server is opened only for a run whose effective `McpTools` policy is
    `Ask` or `Auto`, from one enabled revisioned stdio or Streamable HTTP
    profile. Its
...
- `YOLO` deliberately accepts destructive terminal-action risk within its
  ...
  the live topology changes. Browser YOLO is unavailable and fails closed in
  the host.
